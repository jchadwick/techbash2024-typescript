...
async function init() {
  document
    ...
    .addEventListener("submit", onAddUserFormSubmit);
...
async function fetchUsers(opts = {}) {
  ...
  const response = await fetch(
    `${API_URL}/users?sort=${sortField}`,
  );
  
  const json = await response.json();
  const users = json;
  
  // Runtime data issue NOT detected by TypeScript!
...
function renderUsers(users) {
  ...
  users.forEach((user) => {
    ...
    tr.innerHTML = columns
      .map(
        (field) =>
          `<td>${getUserFieldValue(user, field)}</td>`,
      )
      .join("");
...
function getUserFieldValue(user, field) {
  ...
  try {
    if (field === "type") {
      return metadata.userTypes.find(
        (f) => f.field === value,
      ).name;
    }
...
function toggleAddUserFields(selectedType) {
  document
    ...
    .forEach((field) => {
      const show =
        field.getAttribute("data-type") === selectedType;
      if (show) {
